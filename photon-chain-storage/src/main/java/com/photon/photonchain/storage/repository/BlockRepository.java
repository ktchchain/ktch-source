...
public interface BlockRepository extends CrudRepository<Block, Long> {
    ...
    @Query(value = "select block.blockSignature from Block block where block.blockHeight>:#{#blockHeight}")
    List<String> getSignatureByBlockHeight(@Param("blockHeight") long blockHeight);

    @Modifying
    @Query(value = "delete from BLOCK_BLOCK_TRANSACTIONS where BLOCK_BLOCK_SIGNATURE in(:#{#signatureList})", nativeQuery = true)
    void deleteRelation(@Param("signatureList") List<String> signatureList);

    @Modifying
    @Query(value = "delete from Block block where block.blockHeight>:#{#blockHeight}")
    void deleteBlock(@Param("blockHeight") long blockHeight);
}
